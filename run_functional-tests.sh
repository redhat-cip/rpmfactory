...
function get_image {
    ...
    echo "[+] Get glance image $name: $url" > /dev/stderr
    ...
    echo "${image_uuid}" | tee /dev/stderr
}
...
for h in koji.${DOMAIN} managesf.${DOMAIN}; do
    ...
    ssh -t -t centos@$h "echo '${HOSTS}' | sudo tee -a /etc/hosts"
done
...
echo $(date)
(cd ansible; exec ansible-playbook -vvvv -i /tmp/inventory rpmfactory-and-koji.yml --extra-vars "CN=koji.${DOMAIN}")
...
(cd ansible; exec ansible-playbook -vvvv -i /tmp/inventory rpmfactory-integration-tests.yml --extra-vars "${EXTRA_VAR}")
